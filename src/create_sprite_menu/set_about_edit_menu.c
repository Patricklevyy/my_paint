...
void get_back_from_about_edit_menu(PAINT_T *ALL)
{
...
void manage_display_about_event_edit_menu(PAINT_T *ALL, sfEvent event)
{
...
void display_about_edit_menu(PAINT_T *ALL)
{
    sfEvent event;
    create_about_edit_sprite(ALL);
...
void check_press_about(PAINT_T *ALL, sfEvent event)
{
    if (event.type == sfEvtMouseButtonPressed) {
        display_about_edit_menu(ALL);
    }
}
...
{
    sfVector2i mouse = sfMouse_getPositionRenderWindow(ALL->window->window);
    if (mouse.x >= 851 && mouse.x <= 997) {
        if (mouse.y >= 32 && mouse.y <= 72) {
            check_press_about(ALL, event);
        }
    }
}
